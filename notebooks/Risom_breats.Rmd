...
```
Rather boxplot, otherwise Denis will laugh again
```{r}
...
dev.off()
```
No bigger cell type differences, macbe a bit more tumor cells? Let's compare only bigger categories
...
#final <- final[rowSums(!is.na(final)) > 1,]
#sum(is.na(final[4,]))
...
## second, exclude everything else, and look which interactions can be found there
```{r}

```

How do I look for enrichment between two groups? Look at mean?
```{r, fig.width=30, fig.height=7}
...
# Rather statisticall test for differences, like DESeq
...
dat[is.na(dat)] <- 0
mydata = dat[ , !names(dat_nonprogr) %in% c("Patient_ID", "Status", "Point_Num")]

design <- model.matrix(~0 + dat$Status, data=mydata)

fit <- lmFit(as.matrix(t(mydata)), design)

# Calculate the moderated t-statistics and corresponding p-values using the eBayes() function
fit <- eBayes(fit)

results <- topTable(fit, coef=1, adjust.method="fdr", number=Inf)

sig_features <- subset(results, adj.P.Val < 0.05 & abs(logFC) > 1)
...
model <- aov(data = dat, interaction_measure ~ group)
summary(model)
